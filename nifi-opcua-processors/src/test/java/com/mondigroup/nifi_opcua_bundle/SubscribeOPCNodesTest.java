...
public class SubscribeOPCNodesTest {
    ...
    public void testReal() throws Exception {
        ...
        String tagFilePath = (new File("src/test/resources/subscribeTags.txt")).getAbsolutePath();

        StandardOPCUAService realService = new StandardOPCUAService();

        testRunner.addControllerService("real", realService);

        testRunner.setProperty(realService, StandardOPCUAService.ENDPOINT, "opc.tcp://10.9.112.12:30841/nifi/freeopcua/server/");
        testRunner.setProperty(realService, StandardOPCUAService.USE_PROXY, "true");
        testRunner.setProperty(realService, StandardOPCUAService.PKI_BASE_DIR, "target/opcua-security/pki");

        testRunner.setProperty(realService, StandardOPCUAService.AUTH_POLICY, "Anon");
        //testRunner.setProperty(realService, StandardOPCUAService.USERNAME, "");
